...
public class DictionaryAnnotator extends JCasAnnotator_ImplBase {
        ...
        for (Sentence currSentence : select(jcas, Sentence.class)) {
            ...
            for (int i = 0; i < tokens.size(); i++) {
                ...
                if(extendedMatch.toLowerCase().equals("true")){
                    ...
                    if (longestMatchTokens != null && longestMatchLemmas != null && longestMatchTokens.length == longestMatchLemmas.length) {
    ...
    private String[] findMatch(ArrayList<Token> tokens, List<Token> tokensToSentenceEnd, boolean useLemmas, boolean lowercase) {
        ...
        for (int j = 0; j < tokensToSentenceEnd.size(); j++) {
            ...
            if (lowercase) sentenceToEnd[j] = sentenceToEnd[j].toLowerCase();
        }
        return phrases.getLongestMatch(sentenceToEnd);
    }

    private void annotateMatch(JCas jcas, Type type, Feature f, ArrayList<Token> tokens, int i, String[] longestMatch) {
