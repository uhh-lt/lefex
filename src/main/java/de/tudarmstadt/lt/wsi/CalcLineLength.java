...
public class CalcLineLength extends Configured implements Tool {
    public boolean runJob(String inDir, String outDir) throws Exception {
		...
		job.setJarByClass(CalcLineLength.class);
		FileInputFormat.addInputPath(job, new Path(inDir));
		FileOutputFormat.setOutputPath(job, new Path(_outDir));

		job.setMapperClass(CalcLineLengthMap.class);
		job.setMapOutputKeyClass(LongWritable.class);
		job.setMapOutputValueClass(Text.class);
		job.setReducerClass(NothingReducer.class);

		job.setJobName("NounSenseInduction:CalLimitLength");
	...
	public static void main(final String[] args) throws Exception {
        ...
        int res = ToolRunner.run(conf, new CalcLineLength(), args);
		System.exit(res);
	}
}